...
void GLWidget::drawBlur() {
    ...
    GLint projectionUniformLoc = glGetUniformLocation(m_phongProgram, "projection");
    glUniformMatrix4fv(projectionUniformLoc, 1, GL_FALSE, glm::value_ptr(m_projection));
    // Set model uniform to identity matrix
    GLint modelUniformLoc = glGetUniformLocation(m_phongProgram, "model");

    glm::mat4x4 modelMatrix = glm::translate(glm::vec3(0.0f, 1.2f, 0.0f)); // glm::mat4x4(1.0f)
    glUniformMatrix4fv(modelUniformLoc, 1, GL_FALSE, glm::value_ptr(modelMatrix));

    //Call glViewport so that the viewport is the right size
